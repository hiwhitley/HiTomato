package com.hiwhitley.potatoandtomato.widget.weekview;
...
public class WeekView extends View {
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if ((mEventRects.get(i).event.getDayOfWeek() > -1 && mEventRects.get(i).event.getDayOfWeek() == day)) {
                    ...
                    if (bottom > mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 && left < right &&
                        ...
                        mEventBackgroundPaint.setColor(0x9016BB8E);
    ...
    private String getDayName(int dayOfWeek) {
        if (Calendar.MONDAY == dayOfWeek) return (mDayNameLength == LENGTH_SHORT ? "一" : "周一");
        else if (Calendar.TUESDAY == dayOfWeek) return (mDayNameLength == LENGTH_SHORT ? "二" : "周二");
        else if (Calendar.WEDNESDAY == dayOfWeek) return (mDayNameLength == LENGTH_SHORT ? "三" : "周三");
        else if (Calendar.THURSDAY == dayOfWeek) return (mDayNameLength == LENGTH_SHORT ? "四" : "周四");
        else if (Calendar.FRIDAY == dayOfWeek) return (mDayNameLength == LENGTH_SHORT ? "五" : "周五");
        else if (Calendar.SATURDAY == dayOfWeek) return (mDayNameLength == LENGTH_SHORT ? "六" : "周六");
        else if (Calendar.SUNDAY == dayOfWeek) return (mDayNameLength == LENGTH_SHORT ? "日" : "周日");
